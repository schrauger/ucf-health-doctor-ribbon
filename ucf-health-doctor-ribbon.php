...
function get_ribbon_content() {
	...
	if ( get_field( 'doctor' ) ) {
		...
		$doctor_image_permalink = get_permalink( $doctor->ID );
	} else {
	...
	if ($show_column_3) {
		...
		<div class='wp-block-column'>
			...
			<div class='languages'>
Z				<strong>" . _n("Language", "Languages", (!empty($doctor_languages_array)? sizeof($doctor_languages_array) : 0)) . " Spoken</strong>
				<ul>
					{$language_list}
				</ul>
			</div>   
